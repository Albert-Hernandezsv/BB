...
if($_SESSION["rol"] == "Admin" || $_SESSION["rol"] == "Bodega" || $_SESSION["rol"] == "Facturación"){
} else {
    echo '<script>
...
<br><br>
      ...
      <?php
            ...
            // Definición del arreglo asociativo de unidades
            $unidades = [
                "59" => "Unidad",
                "57" => "Ciento",
                "58" => "Docena",
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control" id="buscarProducto" placeholder="Escribe nombre o código...">
          <div id="resultadosProductos" class="list-group" style="position:relative; z-index:9999;"></div>
        </div>
        ...
        <script>
          $(document).ready(function () {
            ...
            $("#buscarProducto").on("keyup", function () {
              ...
              // si está vacío, limpia resultados
              if (q.length === 0) {
                $("#resultadosProductos").html("");
                return;
              }

              // espera 250ms para no spammear el servidor (debounce)
              timer = setTimeout(function () {

                $.ajax({
                  url: "ajax/productos_buscar.ajax.php",
                  method: "POST",
                  data: { q: q },
                  dataType: "json",
                  success: function (resp) {
                    ...
                    resp.forEach(p => {
                      html += `
                        <a href="#" class="list-group-item list-group-item-action itemProducto"
                          data-id="${p.id}">
                          <strong>${escapeHtml(p.nombre)}</strong>
                          <div style="font-size:12px; opacity:.8;">Código: ${escapeHtml(p.codigo)} | Stock: ${p.stock}</div>
                        </a>
                      `;
                    });
            ...
            // click en un resultado => cargar solo ese producto
            $(document).on("click", ".itemProducto", function (e) {
              e.preventDefault();
              ...
              $.ajax({
                url: "ajax/producto_get.ajax.php",
                method: "POST",
                data: { idProducto: id },
                dataType: "json",
                success: function (p) {
                  ...
                  const tipoTexto = (p.tipo == "1") ? "Bienes"
                                  : (p.tipo == "2") ? "Servicios"
                                  : (p.tipo == "3") ? "Bienes y servicios"
                                  : "Otros";

                  const img = (p.imagen && p.imagen !== "")
                    ? `<img src="${p.imagen}" class="img-thumbnail" width="40px">`
                    : `<img src="vistas/img/anonimo.png" class="img-thumbnail" width="40px">`;
                  ...
                  const row = `
                    <tr>
                      ...
                      <td>${escapeHtml(p.exento_iva)}</td>
                      <td>${escapeHtml(p.peso || "")} ${escapeHtml(p.unidad_texto || "")}</td>
                      <td>${escapeHtml(p.origen || "")}</td>
                      <td>${img}</td>
                      <td>
                        <div class="btn-group">
                          <button class="btn btn-primary btnCodigoBarra" idProducto="${p.id}"><i class="fa fa-barcode"></i></button>
                          <button class="btn btn-warning btnEditarProducto" idProducto="${p.id}" data-toggle="modal" data-target="#modalEditarProducto"><i class="fa fa-pencil"></i></button>
                          <button class="btn btn-danger btnEliminarProducto" idProducto="${p.id}" disabled><i class="fa fa-times"></i></button>
                          <button class="btn btn-info btnAgregarStock" idProducto="${p.id}" data-toggle="modal" data-target="#modalAgregarStock"><i class="fa fa-plus"></i></button>
                          <button class="btn btn-success btnVerAgregacionesStock" idProducto="${p.id}" data-toggle="modal" data-target="#modalVerAgregacionesStock"><i class="fa fa-history"></i></button>
                          <button class="btn btn-secondary btnDuplicarProducto" idProducto="${p.id}" data-toggle="modal" data-target="#modalDuplicarProducto"><i class="fa fa-repeat"></i></button>
                        </div>
                      </td>
                    </tr>
                  `;

                  $("#inventario tbody").html(row);
                }
              });

            });

            // para evitar XSS al pintar texto
              ...
              ($filtroStock == "" || $value["stock"] <= $filtroStock)
                ...
                if($value["tipo"] == "4"){
                  ...
                  echo ' <tr>
                          ...
                          <td>'.$value["peso"].' '.$unidades[$value["unidadMedida"]].'</td>
                          <td>'.$value["origen"].'</td>';
